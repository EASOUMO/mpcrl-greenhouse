...
from agents.sac_agent import train_sac  # CHANGED: import SAC trainer
...
def do_training(
    ...
    """Launches the training of `n_agents` SAC agents in parallel."""
    if isinstance(devices, str):
        devices = (devices,)
    seeds = np.random.SeedSequence(seed).generate_state(n_agents)

    def fun(n: int):
        return train_sac(  # CHANGED: use SAC trainer
            agent_num=n,
            episodes=episodes,
...
def store_data(
    data: dict[str, dict[str, np.ndarray]], identifier: str = "sac"  # CHANGED default
) -> None:
