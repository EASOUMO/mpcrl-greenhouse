...
def train_td3(
    ...
    """Trains a TD3 agent on the greenhouse environment."""
    ...
    action_noise = NormalActionNoise(mean=np.zeros(na), sigma=np.full(na, 0.1))
    ...
    env = model.get_env()
    env.save(f"td3_env_{agent_num}.pkl")
    model.save(f"td3_agent_{agent_num}")

    # return MonitorEpisodes from inside VecNormalize wrapper
